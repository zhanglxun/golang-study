...
// The CWebsiteFunc type is an adapter to allow the use of ordinary
// function as CWebsite mutator.
type CWebsiteFunc func(context.Context, *ent.CWebsiteMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f CWebsiteFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.CWebsiteMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.CWebsiteMutation", m)
}
